...
class ForwardCohort(models.Model):
    """One pre-registered forward experiment across selected 1-second markets."""

    name = models.CharField(max_length=96, default="Forward Cohort")
    engine_version = models.CharField(max_length=16, default="1.3")
    window_ticks = models.PositiveIntegerField(default=25000)
...
class ForwardMarket(models.Model):
    """A market inside a cohort with a fixed no-lookback epoch boundary."""
    ...
    windows_completed = models.PositiveIntegerField(default=0)
...
class ForwardWindow(models.Model):
    """Immutable summary of one non-overlapping 25,000-tick future window."""
    ...
    validated_candidates = models.PositiveIntegerField(default=0)
    live_candidates = models.PositiveIntegerField(default=0)
    results = models.JSONField(default=dict)
    created_at = models.DateTimeField(auto_now_add=True)
